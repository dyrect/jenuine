...
if (!function_exists('mo_breadcrumb')) {
    function mo_breadcrumb() {
        ...
        if (!is_front_page()) {
            ...
            elseif (is_singular(array('portfolio', 'trainer', 'fitness_class', 'feature', 'event'))) {
                $page_id = mo_get_custom_post_type_page();
                if (!empty($page_id)) {
 ...
 */
function mo_get_custom_post_type_page() {
    $page_id = null;
    $options_map = array(
        'portfolio' => 'mo_default_portfolio_page',
        'trainer' => 'mo_trainers_page',
        'fitness_class' => 'mo_fitness_classes_page',
        'feature' => 'mo_features_page',
        'event' => 'mo_events_page'
    );
    $option_id = $options_map[get_post_type()];
    if (!empty($option_id))
        $page_id = mo_get_theme_option($option_id);
